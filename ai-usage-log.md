...
| 3 | 2026-09-25 | Nguyễn Đức Duy | Claude Sonnet 5 | Gợi ý | Spring Security giai đoạn 1 — `backend/src/main/java/com/aigameplatform/backend/config/SecurityConfig.java`, `application.yml` (`app.cors`) | Hướng dẫn từng bước cấu hình `SecurityFilterChain` (CSRF tắt, STATELESS, `permitAll` cho `/api/auth/**` và OPTIONS), bean `PasswordEncoder` (BCrypt) và CORS cho frontend; JWT để làm sau | TODO | TODO | Người dùng tự gõ code theo hướng dẫn; AI kiểm tra và chạy `./mvnw compile` thành công | AI phát hiện người dùng còn thiếu cấu hình `app.cors.allowed-origins` (app sẽ lỗi placeholder khi chạy); đã bổ sung |
| 4 | 2026-09-25 | Nguyễn Đức Duy | Claude Sonnet 5 | Sinh mới | Flyway migration — `backend/src/main/resources/db/migration/V1__enable_extensions.sql`, `V2__create_core_tables.sql`, `application.yml` (flyway baseline) | Viết migration Flyway để thiết lập DB Supabase: V1 bật extension `uuid-ossp`, `vector`; V2 tạo toàn bộ bảng khớp entity (sinh DDL bằng Hibernate offline từ entity rồi chuyển thành migration); bật `baseline-on-migrate` cho schema `public` của Supabase | TODO | TODO | Đã chạy `./mvnw compile` thành công và xác nhận 2 file có trong `target/classes/db/migration`; chưa chạy migration trên DB thật | Đọc nhầm: đề xuất ban đầu của AI chưa lường lỗi Flyway "schema không rỗng nhưng chưa có history" trên Supabase, chỉ phát hiện khi người dùng chạy thật; đã xử lý bằng profile `local` rồi `baseline-on-migrate` |
